...
public class LoansPanel extends javax.swing.JPanel {
    private static Logger logger = Logger.getLogger(LoansPanel.class.getName());
    ...
    /**
     * Runs when the check in/out button ic clicked. Performs the logic for
     * check out(borrowing) or checking in (returning) a book.
     *
     * @param evt The button click event
     */
    private void butCheckActionPerformed(java.awt.event.ActionEvent evt) {
        Borrower borrower = createBorrower();
        String date = tfDate.getText();

        if (driver.dateFormatValid(date)) {
            if (radBorrow.isSelected()) {
                checkBookOut(borrower, date);
            } else if (radReturn.isSelected()) {
                checkBookIn(borrower, date);
            } else {
    ...
    private void checkBookIn(Borrower borrower, String date) {
        Book book = createBook();

        if (book.isOnLoan()) {
            if (borrower.returnBook(book, date)) {
    ...
    private void checkBookOut(Borrower borrower, String date) {
        Book book = createBook();

        if (!book.isOnLoan()) {
            if (borrower.borrow(book, date)) {
                PopUpMessages.infoMessageNormal("\"" + book.getTitle() + "\" loaned successfully to " + borrower.getName());
    ...
    /**
     * Creates a Book based on the fiction/Non-fiction radio button and the
     * book ID
     *
     * @return the Book
     */
    ...
    /**
     * creates a new borrower based on the borrower ID
     * @return the {@link Borrower}
     */
    private Borrower createBorrower() {
        Borrower borrower = null;
        try {
            borrower = new Borrower(Integer.parseInt(tfBorrowerID.getText()));
        } catch (NumberFormatException nfe) {
            PopUpMessages.errorMessageNormal("Please enter numbers only the Borrower ID and Book ID fields.");
        }
        return borrower;
     ...
     */
    private void butViewLoansActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    /**
     * Runs when the add borrower button is clicked. Displays the AddBorrwer GUI
     *
     * @param evt
     */
    private void butAddActionPerformed(java.awt.event.ActionEvent evt) {
        AddBorrower ab = new AddBorrower();
        ab.setVisiblePanel(AddBorrower.BorrowerPanel.ADD_PANEL_CODE);
        ab.setVisible(true);
    }

    /**
     * Runs when the clear button is clicked. Clears all textfields, tables, etc
     *
     * @param evt the button click event
     */
     ...
     * @param evt the button click event
     */
    private void butViewBorrowersActionPerformed(java.awt.event.ActionEvent evt) {
    ...
    private void butUpdateActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        ub.setVisiblePanel(AddBorrower.BorrowerPanel.UPDATE_PANEL_CODE);
        ub.setVisible(true);
    }
    ...
    private void butDeleteActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        db.setVisiblePanel(AddBorrower.BorrowerPanel.DELETE_PANEL_CODE);
        db.setVisible(true);
    }
}
